...
let trans_visitor
  ...
  let alias (ta:Il.typed_addr) : Il.cell =
    let addr, ty = ta in
   ...
   *)
  let alias_cell (c:Il.cell) : Il.cell =
    ...
    then Il.Pcrel (fix, None)
      ...
      let i = Asm.M_POS fix in
        ...
        then Il.Abs i
          ...
          let (reg, _) = force_to_reg (Il.Cell (alias ta)) in
            Il.Based (reg, None)
...
*)
        ...
        begin
          fun proc_cell_alias ->
            trans_upcall Abi.UPCALL_new_proc
              [|
                Il.Cell proc_cell_alias;
                Il.Cell (alias_cell exit_proc_glue_cell);
                Il.Cell (alias_cell fn_cell);
                imm callsz
              |]
        end;
      ()
  ...
  and trans_malloc (dst:Il.cell) (nbytes:int64) : unit =
    aliasing true dst
      begin
        fun dst_alias ->
          trans_upcall Abi.UPCALL_malloc [| Il.Cell dst_alias; imm nbytes |]
      end
  ...
  and trans_recv (initialising:bool) (dst:Ast.lval) (chan:Ast.lval) : unit =
    ...
    let (dstcell, _) = trans_lval dst intent in
      aliasing true dstcell
        begin
          fun dst_alias ->
            trans_upcall Abi.UPCALL_recv [| Il.Cell dst_alias;
  ...
  and trans_init_port (dst:Ast.lval) : unit =
    ...
    let unit_sz = ty_sz abi unit_ty in
      aliasing true dstcell
        begin
          fun dst_alias ->
            trans_upcall Abi.UPCALL_new_port [| Il.Cell dst_alias;
                                                imm unit_sz |]
        end
          ...
          let src = trans_atom atom in
            begin
              match dst_slot.Ast.slot_mode with
                  Ast.MODE_read_alias
                | Ast.MODE_write_alias ->
                    mov dst (Il.Cell (alias (force_to_mem src)))
  ...
  and trans_init_slot_from_cell
    ...
    match dst_slot.Ast.slot_mode with
        Ast.MODE_read_alias
      | Ast.MODE_write_alias -> mov dst (Il.Cell (alias_cell src))
      | _ ->
          trans_copy_slot
            true
            dst dst_slot
